#!/usr/bin/env python3
# StayAwake — Spotify-aware lid-close sleep control (macOS)
# Mirrors Windows logic exactly; platform layer uses caffeinate + AppleScript
# See PROJECT_SPEC.md

HOTKEY_STR = "cmd+shift+space"  # Mac uses Cmd, Windows uses Ctrl — keep same physical keys
...
APPDIR = pathlib.Path.home() / "Library" / "Application Support" / "StayAwake"
STATE_FILE = APPDIR / "state.json"
LOG_FILE = APPDIR / "stayawake.log"
SPOTIFY_AUMID = "com.spotify.client"
...
def is_admin(): return True  # macOS caffeinate doesn't require admin
def ensure_admin(): return True
def hide_console(): pass

# ─── POWER / SLEEP (macOS) ───
def read_lid_values():
    # macOS has no powercfg lid index; we synthesize: 1=Sleep, 0=Do nothing
    # If caffeinate is running, lid is 0, else 1
    return (0,0) if _caffeinate_proc and _caffeinate_proc.poll() is None else (1,1)

def write_lid_values(ac_val, dc_val):
    # No-ops for lid index, real work is caffeinate hold/release
    return True
...
def hold_awake():
    ...
    if _caffeinate_proc and _caffeinate_proc.poll() is None:
        return True
    try:
        # caffeinate -dimsu : prevent display idle, system idle, disk idle, system sleep, user idle
        _caffeinate_proc = subprocess.Popen(["caffeinate", "-dimsu"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        log(f"caffeinate started pid={_caffeinate_proc.pid}")
        return True
    except Exception as e:
        log(f"hold_awake caffeinate failed: {e}", "error")
        return False
...
def release_awake():
    ...
    try:
        if _caffeinate_proc and _caffeinate_proc.poll() is None:
            ...
            try: _caffeinate_proc.wait(timeout=2)
            except: _caffeinate_proc.kill()
...
def do_sleep():
    try:
        log("Forcing sleep via pmset sleepnow", "warning")
...
def startup_recovery():
    ...
    if prev_override and prev_ac is not None and prev_dc is not None:
        log(f"Crash recovery: restoring caffeinate, lid was 0", "warning")
        release_awake()
...
def _play_custom(sound_path):
    try:
        ...
        if p.exists():
            subprocess.run(["afplay", str(p)], timeout=2)
            log(f"Played custom sound {p}")
            return True
    except Exception as e:
        log(f"custom sound failed {sound_path}: {e}", "warning")
    return False
...
def beep_on():
    ...
    try: subprocess.run(["afplay", "/System/Library/Sounds/Pop.aiff"], timeout=1)
    except: print("\a", end="")
def beep_off():
    if not BEEP_ENABLED: return
    try:
    ...
    try: subprocess.run(["afplay", "/System/Library/Sounds/Basso.aiff"], timeout=1)
    except: print("\a", end="")
def beep_error():
    try: subprocess.run(["afplay", "/System/Library/Sounds/Sosumi.aiff"], timeout=1)
    except: pass
...
def get_spotify_raw_status():
    # Returns (raw_status_int_or_None, app_running)
    # On macOS, raw: 4=playing, 5=paused, None=no session
    try:
        # AppleScript check
        r = subprocess.run(["osascript", "-e", 'tell application "Spotify" to player state as string'], capture_output=True, text=True, timeout=3)
...
def is_spotify_playing():
    try:
        ...
        if raw is None:
            ...
            return False
        import psutil  # ensure psutil available
        is_play = (raw == 4)
...
def _read_clamshell():
    try:
        r = subprocess.run(["ioreg", "-r", "-k", "AppleClamshellState", "-d", "2"], capture_output=True, text=True, timeout=3)
        # Look for "AppleClamshellState" = Yes/No
        for line in r.stdout.splitlines():
            if "AppleClamshellState" in line:
                if '"Yes"' in line or "= Yes" in line: return True
                if '"No"' in line or "= No" in line: return False
        return False
    except: return False

def _lid_monitor_thread():
    global lid_closed, lid_known
    log("Lid monitor (ioreg poll) starting")
    while not _exit_flag.is_set():
        try:
            closed = _read_clamshell()
        ...
        for _ in range(30):  # poll every ~3s (30*0.1)
    ...
    try:
        ...
        def on_hotkey():
            ...
            if enabled: beep_on()
                ...
                beep_off()
                if override_active:
                    if original_ac is not None: pass
                    release_awake()
                    override_active=False; pause_start_ts=None; save_state()
        # On Mac, keyboard lib maps cmd to 'command', but we use same string for simplicity
        # Try both variants
        try:         keyboard.add_hotkey("cmd+shift+space", on_hotkey, suppress=True, trigger_on_release=False)
        except: keyboard.add_hotkey(HOTKEY_STR, on_hotkey, suppress=True, trigger_on_release=False)
        keyboard.wait()
    except Exception as e:
...
def main():
    log("="*60)
    log("StayAwake starting — Spotify-aware lid-close control (macOS)")
